function [newX, newWeights] = WEshell(repPerBin, x, weights,binWidth) 
...
bins = -20:binWidth:20; %Specify bins that adequately cover the space
nBins = 40/binWidth + 1; %Specify how many bins exactly that is
...
for n = bins
    nInBin = numel(newX(newX>n-binWidth & newX < n)); %For each bin, find the number of reps in the bin
    xInBin = newX>n-binWidth & newX < n; %Boolean true / false for whether a specific point in the x vector is in the bin
...
%combine the smallest with the first found second smallest.
    if nInBin>0 && nInBin > repPerBin 
        while nInBin > repPerBin
            ...
            %replicas
            nInBin = numel(newX(newX>n-binWidth & newX < n));
            xInBin = newX>n-binWidth & newX < n;
        end
